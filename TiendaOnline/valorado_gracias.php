...
if($_SERVER['REQUEST_METHOD'] ==='POST'){
    ...
    if ($_POST['accion']==='Valorar Producto'){
        
        if(empty($_POST['COMENTARIO']))
            exit('Completar el campo valoración');

        if(empty($_POST['PUNTUACION']))
            exit('Completar el campo puntuación');

        if(!is_numeric($_POST['PUNTUACION']))
            exit('Completar con datos de tipo entero/o con decimales');

        if(empty($_POST['USUARIO_ID_USUARIO']))
            exit('Error con el campo id usuario');

        if(empty($_POST['PRODUCTO_ID_PRODUCTO']))
            exit('Error con el campo id producto');
...
<body>
    ...
    <div class="container" id="main">
        <div class="row">
            <div class="jumbotron">
                <p>Has valorado el producto satisfactoriamente, gracias.</p>
                <p>
